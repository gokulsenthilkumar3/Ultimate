...
const AnalyticsTab = React.memo(function AnalyticsTab({ COLORS, fmtINR, form, pieData, TOOLTIP_STYLE, expenses, onCategorySelect }: any) {
  const [activeIndex, setActiveIndex] = useState(0);
  const onPieEnter = (_: any, index: number) => setActiveIndex(index);
  const hasSpendData = Number(expenses) > 0 && pieData.length > 0;
    ...
    <div className="dual-grid">
          ...
          <div className="glass-card">
            ...
            {hasSpendData ? (
              ...
              <EmptyState icon="BarChart2" title="Nothing to chart" description="Your monthly spend velocity will appear here after the first expense." />
            )}
          </div>
        </div>
  );
