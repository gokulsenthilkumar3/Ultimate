...
class QuantileRegressionLayer(_BaseLayer):
    """
    Output layer that predicts Q10, Q50 (median), and Q90 simultaneously.

    Used in build_quantile_model() to produce calibrated prediction intervals.
    The custom quantile_loss must be passed to model.compile().
    """

    def __init__(self, quantiles: tuple = (0.1, 0.5, 0.9), **kwargs):
        super().__init__(**kwargs)
    ...
    def build(self, input_shape):
        self.w = self.add_weight(
            name="q_weights", shape=(input_shape[-1], self.n_q),
            initializer="glorot_uniform", trainable=True,
        )
        self.b = self.add_weight(
            name="q_bias", shape=(self.n_q,), initializer="zeros", trainable=True,
        )
        super().build(input_shape)

    def call(self, inputs):
        return tf.matmul(inputs, self.w) + self.b  # (B, n_q)

    @staticmethod
    def quantile_loss(y_true: "tf.Tensor", y_pred: "tf.Tensor",
                      quantiles: tuple = (0.1, 0.5, 0.9)) -> "tf.Tensor":
        """
        Pinball / quantile loss averaged over all quantile heads.
        y_true shape: (B, 1) or (B,); y_pred shape: (B, n_q).
        """
        losses = []
        y_flat = tf.cast(tf.reshape(y_true, [-1]), tf.float32)
        for i, q in enumerate(quantiles):
            pred  = tf.cast(y_pred[:, i], tf.float32)
            error = y_flat - pred
            loss  = tf.where(error >= 0, q * error, (q - 1.0) * error)
...
def build_attention_interpretable(
    timesteps: int,
    n_features: int,
    d_model: int = 64,
    num_heads: int = 8,
    ...
    """
    Interpretable Multi-Head Attention model.

    Exposes attention weights via a secondary output so callers can understand
    which historical timesteps drove each prediction. During normal training
    use only the ``rate_output``. For explainability call with ``return_attention``
    using ``model.get_layer('mha').weights``.

    Architecture
    ------------
    Input → Dense(d_model) → PositionalEncoding → [MHA + Pre-LN] × 2
          → GAP → Dense(64, gelu) → Dropout → Dense(1)
    """
    if not HAS_TF:
        raise ImportError("TensorFlow is required for build_attention_interpretable.")

    # Align d_model to be divisible by num_heads
    d_model = max(num_heads, (d_model // num_heads) * num_heads)

    inp   = Input(shape=(timesteps, n_features), name="sequence_input")
    x     = Dense(d_model, name="input_proj")(inp)
    x     = PositionalEncoding(name="pos_enc")(x)
    x     = Dropout(dropout)(x)

    # Two interpretable transformer blocks with Pre-LN
    x = TransformerBlock(num_heads, d_model, ff_dim, dropout, name="tf_block_1")(x)
    x = TransformerBlock(num_heads, d_model, ff_dim, dropout, name="tf_block_2")(x)

    x     = LayerNormalization(epsilon=1e-6)(x)
    x     = GlobalAveragePooling1D()(x)
    x     = Dense(64, activation="gelu")(x)
    x     = Dropout(dropout)(x)
    out   = Dense(1, name="rate_output")(x)

    m = Model(inp, out, name="Attention-Interp")
    m.compile(optimizer=_OPT(), loss=Huber(), metrics=["mae"])
    return m
...
def build_quantile_model(
    ...
    """
    Quantile Regression LSTM model for prediction interval estimation.

    Outputs three values per prediction (Q10, Q50, Q90) simultaneously.
    The model must be compiled with ``QuantileRegressionLayer.quantile_loss``.

    Architecture
    ------------
    Input → LSTM(128) → Dropout → LSTM(64) → Dropout
          → Dense(32, relu) → QuantileRegressionLayer([0.1, 0.5, 0.9])

    Usage
    -----
    >>> model = build_quantile_model(15, n_features)
    >>> quantile_preds = model.predict(X_seq)   # shape (N, 3)
    >>> lower, median, upper = quantile_preds[:, 0], quantile_preds[:, 1], quantile_preds[:, 2]
    """
    if not HAS_TF:
        raise ImportError("TensorFlow is required for build_quantile_model.")
    ...
    x    = Dropout(0.2)(x)
    x    = Dense(32, activation="relu")(x)
    out  = QuantileRegressionLayer(quantiles=quantiles, name="quantile_output")(x)

    m = Model(inp, out, name="QuantileLSTM")
    loss_fn = lambda y_true, y_pred: QuantileRegressionLayer.quantile_loss(
        y_true, y_pred, quantiles=quantiles
    )
    m.compile(optimizer=_OPT(), loss=loss_fn, metrics=["mae"])
    return m


def mc_predict_with_quantiles(
    ...
    """
    Run the QuantileLSTM model and aggregate outputs.

    Returns
    -------
    dict with keys:
      lower_bound  : np.ndarray (N,) — Q10 percentile
      prediction   : np.ndarray (N,) — Q50 median prediction
      upper_bound  : np.ndarray (N,) — Q90 percentile
      std_estimate : np.ndarray (N,) — (Q90 - Q10) / 2.56 ~ 1-sigma estimate
    """
    all_outputs = []
    for _ in range(n_samples):
        ...
        for start in range(0, len(X), batch_size):
            ...
            out   = quantile_model(batch, training=True)  # dropout active
            parts.append(out.numpy())
        all_outputs.append(np.concatenate(parts, axis=0))  # (N, 3)

    stacked = np.stack(all_outputs, axis=0)  # (S, N, 3)
    mean_q  = stacked.mean(axis=0)            # (N, 3)
    return {
        "lower_bound":  mean_q[:, 0],
        "prediction":   mean_q[:, 1],
        "upper_bound":  mean_q[:, 2],
        "std_estimate": (mean_q[:, 2] - mean_q[:, 0]) / 2.56,
    }
...
if HAS_TF:
    DL_BUILDERS = {
        ...
        "TFT":              build_tft,
        "TCN":              build_tcn,
        "Attention-Interp": build_attention_interpretable,
    }
    # Quantile model is registered separately — it has a different output shape
    # and loss function; it is NOT included in the main DL_BUILDERS loop.
    QUANTILE_BUILDER = build_quantile_model
else:
    DL_BUILDERS = {}
    QUANTILE_BUILDER = None
