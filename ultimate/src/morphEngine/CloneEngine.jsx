...
function TimelineClone() {
  ...
  useEffect(() => {
    ...
    return use3DStore.subscribe(
      ...
      () => {
        const scrubbedState = getScrubbedMorphState();
        setCurrentMetrics(scrubbedState.metrics);
      }
    );
  }, [setCurrentMetrics, getScrubbedMorphState]);
...
export default function CloneEngine() {
  ...
  switch (viewMode) {
    ...
    // ── SPLIT MODE ────────────────────────────────────────────────────────────
    // Both models at same position; visual split is a CSS/HTML overlay.
    // True stencil masking requires a custom renderer pass (Layer 4 extension).
      ...
      return (
        <>
          ...
          <HumanoidClone
            ...
            showAura={false}
          />
          <SplitDivider dividerX={splitDividerX} />
        </>
      );
