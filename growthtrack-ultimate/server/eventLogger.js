  ...
  const write = async (input, req = null) => {
    ...
    try {
      ...
      if (event.category === 'auth') {
        await prisma.loginLog.create({ data: { user_id: event.user_id, email: event.user_email, action: event.action, failure_reason: event.details, ip_address: event.actor_ip, user_agent: event.user_agent, source: event.source, request_id: event.request_id, metadata: event.metadata } });
      } else if (event.category === 'session') {
        await prisma.sessionLog.create({ data: { user_id: event.user_id, action: event.action, details: event.details, ip_address: event.actor_ip, user_agent: event.user_agent, source: event.source, request_id: event.request_id, metadata: event.metadata } });
      } else {
        await prisma.auditLog.create({ data: common });
      }
