...
function main() {
  ...
  for (const meshItem of json.meshes ?? []) {
    for (const primitive of meshItem.primitives ?? []) {
      Object.values(primitive.attributes ?? {}).forEach((v) => usedAccessors.add(v));
      if (primitive.indices != null) usedAccessors.add(primitive.indices);
      for (const target of primitive.targets ?? []) {
        Object.values(target).forEach((v) => usedAccessors.add(v));
      }
    }
  }
  ...
  const usedBufferViews = new Set([...usedAccessors].map((idx) => json.accessors?.[idx]?.bufferView).filter((v) => v != null));
  ...
  check((skin?.joints?.length ?? 0) >= 40, 'Joint count', `${skin?.joints?.length ?? 0} joints`, failures);
