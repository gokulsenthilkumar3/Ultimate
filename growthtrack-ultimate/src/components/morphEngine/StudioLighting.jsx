...
const LIGHTING_BY_TIER = {
  HIGH: { key: 48, fill: 2.8, point: 2.4, edge: 30, edgeSecondary: 18, warmRim: 20, hair: 28, backlight: 10, ambient: 0.08, hemisphere: 0.28 },
  MED:  { key: 42, fill: 2.2, point: 1.8, edge: 24, edgeSecondary: 13, warmRim: 14, hair: 18, backlight: 7, ambient: 0.10, hemisphere: 0.32 },
  LOW:  { key: 34, fill: 1.5, point: 1.2, edge: 0, edgeSecondary: 0, warmRim: 0, hair: 0, backlight: 0, ambient: 0.16, hemisphere: 0.40 },
};
...
function EdgeLights({ profile, lighting }) {
  ...
  return (
    <>
      ...
      {/* Hair/kicker light — top-down warm, separates head from background */}
      {lighting.hair > 0 && (
        <spotLight position={[0.4, 5.2, -1.8]} intensity={lighting.hair} color="#fff5e0" distance={8} decay={2} angle={0.38} penumbra={0.92} />
      )}
      {/* Subtle backlight — warm amber adds depth behind the models */}
      {lighting.backlight > 0 && (
        <pointLight position={[0, 1.5, -3.8]} intensity={lighting.backlight} color="#ffe4c0" distance={7} decay={2} />
      )}
    </>
  );
}
