...
export const MORPH_RANGES = Object.freeze({
  ...
  shoulderSlope: { min: 0, max: 1 },
  shoulderDrop: { min: 0, max: 1 },
...
export function computeMorphWeights(metrics = {}, inheritedMetrics = {}) {
  ...
  return constrainMorphWeights({
    ...
    // Width, slope and vertical drop are independent measurements. Mapping a
    // broad shoulder directly to a near-maximum drop pinches the arm socket.
    shoulder_slope: renderMetrics.shoulderSlope == null ? 0.16 : normalise(renderMetrics.shoulderSlope, 'shoulderSlope'),
