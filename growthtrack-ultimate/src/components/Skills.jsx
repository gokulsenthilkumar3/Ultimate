...
import { BookOpen, Award, Target, Plus, Search, TrendingUp, Save, Trash2, X, Zap, Star, ChevronUp } from 'lucide-react';
  ...
  return (
    <div className={compact ? 'space-y-0.5' : 'space-y-1.5'}>
      ...
      <div className="w-full rounded-full overflow-hidden" style={{ height: compact ? '5px' : '8px', background: 'rgba(255,255,255,0.08)' }}>
        <div
          className={`h-full rounded-full bg-gradient-to-r ${lvl.xpColor} transition-all duration-500`}
          style={{ width: `${proficiency}%` }}
        />
      </div>
      {!compact && next && (
        <p className="text-xs text-gray-500">{progress}% to Lv.{next.level} {next.label}</p>
      )}
      {!compact && !next && (
        <p className="text-xs" style={{ color: '#ffd700' }}>✨ MAX LEVEL</p>
      )}
    </div>
  );
}
  ...
  return (
    <div className="bg-white/5 border border-white/10 rounded-xl p-4">
      ...
      <svg width="220" height="220" viewBox="0 0 220 220" className="mx-auto block">
        ...
        {/* Value polygon */}
        <polygon
          points={polyStr(valuePoints)}
          fill="rgba(99,102,241,0.15)" stroke="rgba(99,102,241,0.7)" strokeWidth="1.5"
        />
        {/* Labels */}
        {entries.map(([cat, avg], i) => {
          ...
          return (
            <text key={cat} x={lx} y={ly} textAnchor="middle" dominantBaseline="middle"
              fontSize="8" fontWeight="700" fill={lvl.color}>
              {cat.slice(0, 10)}
            </text>
          );
        })}
...
export default function Skills() {
  ...
  return (
    <div className="fade-in space-y-6 p-4">
      ...
      {categories.filter(c => c !== 'All' && (activeCategory === 'All' || c === activeCategory)).map(cat => {
        ...
        return (
          <div key={cat} className="space-y-3">
            <div className="flex items-center gap-3 pb-1 border-b border-white/8">
              <h3 className="text-base font-bold" style={{ color: catLvl.color }}>{cat}</h3>
                ...
                return (
                  <div
                    ...
                    className={`relative rounded-xl border p-4 flex flex-col gap-3 transition ${
                      isLevelingUp ? 'border-amber-400/60 bg-amber-500/8 shadow-lg shadow-amber-500/10' : 'border-white/10 bg-white/4 hover:bg-white/6'
                    }`}
                  >
                    {isLevelingUp && (
                      <div className="absolute -top-2 -right-2 bg-amber-400 text-black text-xs font-black px-2 py-0.5 rounded-full shadow animate-pulse">
                        ⚡ LVL UP!
                      </div>
                    )}
                    ...
                    <div className="flex items-start gap-3 pr-8">
                      <input
                        ...
                        className="w-10 h-10 text-xl text-center bg-white/8 border border-white/10 rounded-xl flex-shrink-0"
                        title="Emoji"
                      />
