 ...
 *
 * FIXES (2026-05-16):
 *   Fix 1 — Removed <Environment preset="studio" /> (drei Environment triggers
 *            PMREMGenerator/CubeCamera in three.js 0.184 which crashes the
 *            WebGL context with "Cannot read properties of undefined (reading
 *            'length')"). Scene lighting is handled by the directional + ambient
 *            rig already present in SystemScene — no Environment needed.
 *
 *   Fix 2 — GLB paths now use import.meta.env.BASE_URL so the /Ultimate/ prefix
 *            is included on GitHub Pages (was resolving to root / and 404-ing).
 *            FALLBACK_GLB (threejs.org Soldier) kept for local dev without model.
 *
 *   Fix 3 — Added null-guard on morphTargetInfluences before accessing .length
 *            inside useFrame traverse to prevent downstream crash when the mesh
 *            is mid-load or the morph dictionary is absent.
 *
 * UPGRADE LOG:
...
// ── FIX 2: Base URL for GitHub Pages (/Ultimate/) ──
...
// Fallback for when local GLB is missing (e.g. dev without model file)
